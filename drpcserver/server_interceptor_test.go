...
func (m *mockStream) MsgRecv(_ drpc.Message, _ drpc.Encoding) error {
	if len(m.msgRecvd) > 0 {
		// Simplified: actual unmarshaling is more complex.
		// Example: reflect.ValueOf(msg).Elem().Set(reflect.ValueOf(m.msgRecvd[0]).Elem())
		// m.msgRecvd = m.msgRecvd[1:]
		return nil // Placeholder for successful receive
	}
