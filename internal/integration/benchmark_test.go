...
func benchmarkParallelActiveStreamsModes(b *testing.B, writeLatency time.Duration) {
	...
	modes := []struct {
		...
		{"NonMux", false, true},
		{"Mux/ConnPerStream", true, true},
		{"Mux/SharedConn", true, false},
	}
				...
				b.Run("streams="+strconv.Itoa(streamCount), func(b *testing.B) {
					for _, transport := range transports {
						for _, mode := range modes {
							b.Run(transport.name+"/"+mode.name, func(b *testing.B) {
								benchmarkParallelActiveStreams(b, sz.data, streamCount, mode.mux, mode.oneConnPerStream, writeLatency, transport.useTCP)
							})
						}
					}
				})
...
func benchmarkParallelActiveStreams(
	b *testing.B, payload []byte, streamCount int, mux bool, oneConnPerStream bool, writeLatency time.Duration, useTCP bool,
) {
	...
	opts := connTransportOpts{
		writeDelay: writeLatency,
		useTCP:     useTCP,
		mux:        mux,
	}
	...
	if !oneConnPerStream {
		sharedConn = createConnectionWithTransport(b, echoServer, ctx, opts)
	}

	for i := 0; i < streamCount; i++ {
		conn := sharedConn
		if oneConnPerStream {
			conn = createConnectionWithTransport(b, echoServer, ctx, opts)
		}
